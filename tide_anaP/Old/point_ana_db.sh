        ...
        if [[ $OBS_FLAG != 0 ]]; then
           ...
           if [[ $DIFF_FLAG == 1 ]]; then
              ...
              if [[ ${MOD1_FLAG} == 1 ]]; then
                 ...
                 while read OLINE ; do
                   if [[ ${OLINE} != "#" ]]; then
                      ODATA_OTIME=$( echo $OLIBE | cut -f 2 -d: )
                      #OTIME=$( echo $OLIBE | cut )
                      OVAL=$( echo $OLIBE | cut -f 5 -d: )
                      MVAL=$( grep "$ODATA_TIME" $TSPLOT_INFILE_0 | grep ":00:00" )
                      echo "$MVAL $( echo $MVAL | cut -f 1,2,3,4 -d: ) $OVAL " >> $INFILE_PASTE
                   fi
                 done < $TSPLOT_INFILE_OBS
              else
                 paste $TSPLOT_INFILE_0 $TSPLOT_INFILE_1 > paste_temp.txt
                 #paste $TSPLOT_INFILE_0 $TSPLOT_INFILE_1 $TSPLOT_INFILE_OBS > $INFILE_PASTE
                 while read OLINE ; do
                   if [[ ${OLINE} != "#" ]]; then
                      ODATA=$( echo $OLIBE | cut )
                      OTIME=$( echo $OLIBE | cut )
                      OVAL=$( echo $OLIBE | cut )
                      MVALS=$( grep "$ODATA $OTIME " paste_temp.txt )
                      echo "$ MVALS .. " >> $INFILE_PASTE
                   fi
                 done < $TSPLOT_INFILE_OBS
 ...
 if [[ $MYDIAG_FLAG == 1 ]]; then
    ...
    for MYDIAG_FIELD in ${MYDIAG_SHORT_NAMES[@]}; do
        ...
        if [[ -f $MYDIAG_FILE_1 ]] && [[ -f $MYDIAG_FILE_2 ]] ; then
          ...
          cdo sub $MYDIAG_FILE_1 $MYDIAG_FILE_2 $MYDIAG_DIFFOUT 
          ...
          GNUPLOT_MYDIAG_TMP="mydiag_tmp.gpl"

          echo "#" > ${GNUPLOT_MYDIAG_TMP}
          echo "set term jpeg size 1500,900" >> ${GNUPLOT_MYDIAG_TMP}
          ...
          echo "plot '$MYDIAG_PLOTINFILE_1' using 1:3 with line lw 2 lt rgb \"blue\" title gprintf(\"${MYDIAG_INTAG_1} (AVG = %g [${MYDIAG_PLOTUDM}] ) \", STATS1_mean), '$MYDIAG_PLOTINFILE_2' using 1:3 with line lw 2 lt rgb \"red\" title gprintf(\"${MYDIAG_INTAG_2} (AVG = %g [${MYDIAG_PLOTUDM}] )\", STATS2_mean) " >> ${GNUPLOT_MYDIAG_TMP}
          echo "plot '$MYDIAG_PLOTINFILE_DIFF' using 1:3 with line lw 2 lt rgb \"black\" title \"Diff: ${MYDIAG_INTAG_1} - ${MYDIAG_INTAG_2}\" " >> ${GNUPLOT_MYDIAG_TMP}
