 ...
 if [[ ${#F_NUM[@]} -lt 24 ]]; then
    ...
    while [[ $IDX_DATE -lt $ANA_ENDDATE ]]; do
          ...
          while [[ $H_IDX -lt $MAX_HOUT  ]]; do
                ...
                PRE_PREV_HOUR=$(( $PREV_HOUR - 1 ))
                PREV_HOUR=$( printf %02d $PREV_HOUR )
                PRE_PREV_HOUR=$( printf %02d $PRE_PREV_HOUR )
                POST_FILE_LOWVAL=$(  echo "${ANA_INFILE_TPL}_${PREV_HOUR}.nc${VAR_NAME}" | sed -e "s/%YYYYMMDD%/$PREV_DAY/g" )
                FILE_LOWVAL=${POST_FILE_LOWVAL}_00
                PRE_FILE_LOWVAL=$(  echo "${ANA_INFILE_TPL}_${PRE_PREV_HOUR}.nc${VAR_NAME}" | sed -e "s/%YYYYMMDD%/$PREV_DAY/g" )

                # Interpolation from :30h to :00h 
                cdo mergetime $PRE_FILE_LOWVAL $POST_FILE_LOWVAL merged_PREV.nc
                cdo timmean merged_PREV.nc $FILE_LOWVAL
                rm -v merged_PREV.nc
 
                SUCC_DAY=$( date -d "$IDX_DATE 1 day" +%Y%m%d )
                SUCC_HOUR=$( printf %02d $SUCC_HOUR )
                PRE_SUCC_HOUR=$(( $SUCC_HOUR - 1 ))
                PRE_SUCC_HOUR=$( printf %02d $PRE_SUCC_HOUR )
                POST_FILE_HIVAL=$(  echo "${ANA_INFILE_TPL}_${SUCC_HOUR}.nc${VAR_NAME}" | sed -e "s/%YYYYMMDD%/$SUCC_DAY/g" )
                FILE_HIVAL=${POST_FILE_HIVAL}_00
                PRE_FILE_HIVAL=$( echo "${ANA_INFILE_TPL}_${PRE_SUCC_HOUR}.nc${VAR_NAME}" | sed -e "s/%YYYYMMDD%/$SUCC_DAY/g" )
               
                # Interpolation from :30h to :00h 
                cdo mergetime $PRE_FILE_HIVAL $POST_FILE_HIVAL merged_SUCC.nc
                cdo timmean merged_SUCC.nc $FILE_HIVAL
                rm -v merged_SUCC.nc
          ...
          if [[ $H_IDX == 7 ]]; then
             ...
             echo "Computing the ${H_IDX} element of Doodson filter (00 case).."
                PREV_DAY=$IDX_DATE
                PRE_PREV_DAY=$( date -d "$PREV_DAY -1 day" +%Y%m%d )
                PREV_HOUR="00"
                PRE_PREV_HOUR="23"
                POST_FILE_LOWVAL=$(  echo "${ANA_INFILE_TPL}_${PREV_HOUR}.nc${VAR_NAME}" | sed -e "s/%YYYYMMDD%/$PREV_DAY/g" )
                FILE_LOWVAL=${POST_FILE_LOWVAL}_00
                PRE_FILE_LOWVAL=$(  echo "${ANA_INFILE_TPL}_${PRE_PREV_HOUR}.nc${VAR_NAME}" | sed -e "s/%YYYYMMDD%/$PRE_PREV_DAY/g" )
                ...
                SUCC_DAY=$( date -d "$IDX_DATE 1 day" +%Y%m%d )
                PRE_SUCC_DAY=$( date -d "$SUCC_DAY -1 day" +%Y%m%d )
                SUCC_HOUR="00"
                PRE_SUCC_HOUR="23"
                POST_FILE_HIVAL=$(  echo "${ANA_INFILE_TPL}_${SUCC_HOUR}.nc${VAR_NAME}" | sed -e "s/%YYYYMMDD%/$SUCC_DAY/g" )
                FILE_HIVAL=${POST_FILE_HIVAL}_00
                PRE_FILE_HIVAL=$(  echo "${ANA_INFILE_TPL}_${PRE_SUCC_HOUR}.nc${VAR_NAME}" | sed -e "s/%YYYYMMDD%/$PRE_SUCC_DAY/g" )
          ...
          while [[ $H_IDX -lt ${#F_NUM[@]}  ]]; do
                ...
                PRE_PREV_HOUR_2=$( printf %02d $PRE_PREV_HOUR )
                POST_FILE_LOWVAL=$(  echo "${ANA_INFILE_TPL}_${PREV_HOUR_2}.nc${VAR_NAME}" | sed -e "s/%YYYYMMDD%/${IDX_DATE}/g" )
                FILE_LOWVAL=${POST_FILE_LOWVAL}_00
                PRE_FILE_LOWVAL=$(  echo "${ANA_INFILE_TPL}_${PRE_PREV_HOUR_2}.nc${VAR_NAME}" | sed -e "s/%YYYYMMDD%/${IDX_DATE}/g" )
                ...
                PRE_SUCC_HOUR_2=$( printf %02d $PRE_SUCC_HOUR )
                POST_FILE_HIVAL=$(  echo "${ANA_INFILE_TPL}_${SUCC_HOUR_2}.nc${VAR_NAME}" | sed -e "s/%YYYYMMDD%/${IDX_DATE}/g" )
                FILE_HIVAL=${POST_FILE_HIVAL}_00
                PRE_FILE_HIVAL=$(  echo "${ANA_INFILE_TPL}_${PRE_SUCC_HOUR_2}.nc${VAR_NAME}" | sed -e "s/%YYYYMMDD%/${IDX_DATE}/g" )
